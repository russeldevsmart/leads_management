...
router.get("/get-dashboard-info", async (req, res) => {
  ...
  const topLeadAction = await Action.find({}).sort({ date: -1 }).limit(1).populate("lead").populate("user");
  
  const topLead = topLeadAction && topLeadAction.length >= 1 ? topLeadAction[0].user.fullname : null;
